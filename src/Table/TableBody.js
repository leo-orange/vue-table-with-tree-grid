...
export default {
  ...
  methods: {
    toggleStatus(type, row, rowIndex, value) {
     ...
     //console.log({[`_is${type}`]: typeof value === 'undefined' ? !row[`_is${type}`] : value});
     console.log(this.table.bodyData);
     //alert(this.table.bodyData.length);
      this.table.bodyData.splice(rowIndex, 1, {
        ...target,
        [`_is${type}`]: typeof value === 'undefined' ? !row[`_is${type}`] : value,
      });
    },
    toggleStatus1(type, row, rowIndex, value) {
      this.validateType(type, ['Expanded', 'Checked', 'Hide', 'Fold'], 'toggleStatus1', false);
      const target = this.table.bodyData[rowIndex];
      //console.log({[`_is${type}`]: typeof value === 'undefined' ? !row[`_is${type}`] : value});
    //console.log('a'+this.table.bodyData);
      //alert(this.table.bodyData.length);
     this.table.bodyData.splice(rowIndex, 1,{ ...target, [`_is${type}`]: typeof value === 'undefined' ? !row[`_is${type}`] : value,});

      //console.log(JSON.stringify(this.table.bodyData[rowIndex]));
      //this.table.bodyData[rowIndex]._isChecked=true;
      //this.table.bodyData[rowIndex]._isFold=true;

      /*this.table.bodyData=JSON.parse("[{\n" +
        "\t\"_isHover\": true,\n" +
        "\t\"_isExpanded\": false,\n" +
        "\t\"_isChecked\": true,\n" +
        "\t\"_level\": 1,\n" +
        ...
        "\t\"score\": 10\n" +
        "}]")*/
      console.log('aaaaa'+JSON.stringify(this.table.bodyData));
    },
    ...
    handleEvent1($event, type, data, others) {
      const certainType = this.validateType(type, ['cell', 'row', 'checkbox', 'icon'], 'handleEvent');
      const eventType = $event ? $event.type : '';
      const { row, rowIndex, column, columnIndex } = data;
      const latestData = this.table.bodyData;
      ...
      if (certainType.checkbox) {
        const { isChecked } = others;
       this.toggleStatus1('Checked', row, rowIndex, isChecked);
        /*if (row._childrenLen > 0) {
          const childrenIndex = this.getChildrenIndex(row._level, rowIndex, false);
          for (let i = 0; i < childrenIndex.length; i++) {
            this.toggleStatus1('Checked', latestData[childrenIndex[i]], childrenIndex[i], isChecked);
          }
        }*/
        return this.table.$emit('checkbox-click', latestData[rowIndex], column, columnIndex, $event);
      }
    ...
    function renderCell(row, rowIndex, column, columnIndex) {
      ...
      if (this.isSelectionCell(this.table, columnIndex)) {
        ...
        return <Checkbox
          ...
          value={ allCheck }
          onOn-change={ isChecked => this.handleEvent1(null, 'checkbox', { row, rowIndex, column, columnIndex }, { isChecked }) }>
          </Checkbox>;
      }
      // Tree's firstProp
